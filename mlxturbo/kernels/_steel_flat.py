"""MLX 0.32.2 の steel GEMM / 量子化ローダーを 1 本のヘッダ文字列に平坦化した写し。

``mx.fast.metal_kernel`` は ``newLibrary(source)`` に文字列を渡すだけで
インクルードパスを持たないので、``#include "mlx/..."`` が書けない
(``mlx/backend/metal/custom_kernel.cpp:53``)。steel を使うには必要な
template / struct を全部この 1 本に並べるしかない。

**このファイルは手で書いたものではない。**``~/dev/mlx-serve/lib/mlx-src`` の
MLX 0.32.2 (MIT, Copyright (c) 2023 Apple Inc.) から、以下の範囲をそのまま
機械的に抜き出して連結してある (転記ミスでビット一致が壊れるのを避けるため)。

  - ``kernels/steel/utils/type_traits.h``   全体 (``pointer_element_t``)
  - ``kernels/steel/utils/integral_constant.h`` 全体 (``Int<>``)
  - ``kernels/steel/gemm/transforms.h``     全体 (``TransformNone``)
  - ``kernels/steel/gemm/mma.h``            19-428 行 + 439-751 行
    (``BaseMMAFrag`` / ``MMATile`` / ``tile_matmad`` / ``BlockMMA``。
     ``complex64_t`` 特殊化 429-437 行と 753-1156 行は落とした)
  - ``kernels/steel/gemm/loader.h``         全体 (``BlockLoader``)
  - ``kernels/quantized.h``                 12-26 行 (``get_pack_factor`` ほか)、
    483-569 行 (``dequantize``)、571-698 行 (``QuantizedBlockLoader``)、
    1192-1318 行 (``qmm_t_impl``)
  - ``kernels/quantized_utils.h``           6-30 行 (``gemm_loop_aligned``)

各ファイル冒頭の ``#pragma once`` / ``#include "mlx/..."`` / system header /
``using namespace metal;`` だけを外し、残りは 1 文字も変えていない。
``align_M`` / ``align_N`` / ``align_K`` の ``function_constant``
(``quantized.h:6-8``) は custom kernel から使えないので写していない
(``qmm_t_impl`` は template 引数の ``aligned_N`` しか見ないので不要)。

MLX 側を更新したら、上の行範囲を取り直してこの写しを作り直すこと
(``#pragma once`` / ``#include`` / ``using namespace metal;`` を落として
順に連結するだけ。写した各塊は波括弧が閉じていること)。
"""
...
// ---- steel/utils/type_traits.h (pointer_element_t) ----
...
// ---- steel/gemm/transforms.h (TransformNone / AccumHelper) ----
///////////////////////////////////////////////////////////////////////////////
...
// ---- steel/gemm/mma.h (BaseMMAFrag / MMATile / tile_matmad / BlockMMA) -- complex64_t 特殊化と NAX は除く ----
namespace mlx {
namespace steel {
...
// ---- steel/gemm/loader.h (BlockLoader) ----
///////////////////////////////////////////////////////////////////////////////
// Loading helper
///////////////////////////////////////////////////////////////////////////////
...
// ---- quantized.h:12-26 (get_pack_factor / get_bytes_per_pack) ----
...
// ---- quantized.h:483-569 (dequantize) ----
...
// ---- quantized.h:571-698 (QuantizedBlockLoader) ----
template <
    typename T,
    short BROWS,
    short BCOLS,
...
// ---- quantized_utils.h:6-30 (gemm_loop_aligned) ----
...
// ---- quantized.h:1192-1318 (qmm_t_impl) ----
template <
    typename T,
