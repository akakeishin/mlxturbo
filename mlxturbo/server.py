...
def main() -> None:
    ...
    ap.add_argument(
        ...
        " 指定してもよい。まとめる条件は「サンプリング設定が同じ」ことだけで、"
        " 外れた要求は従来どおり直列に流れる (長さの上限は 2026-09-02 に外れた。"
        " QSA のブロック境界を行ごとに引き直したので、以前の"
        " 「プロンプト長 + max_tokens が indexer_budget (2048) に収まる」は要らない"
        " -- mlxturbo/batch_spec.py の _ragged_indexer_call 参照)。"
        " スケジューラは chunked prefill 方式で、新しい"
