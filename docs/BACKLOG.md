...
2026-09-04時点の全項目の実行状態は`docs/BACKLOG-AUDIT-2026-09-04.md`を入口にする。
...
14. **解決済み (2026-09-04): `enable_hc_write` の関数内envゲート。**
    `MLXTURBO_HC_WRITE=0`なら直接呼んでも差し替えない。runner経由の既存ゲートに
    加えて関数自身でも守り、合成10 testで既定onと明示offを確認した。
...
64-token出力が完全一致し、関連437 testも通った。次は Gemma の KV量子化。

## 畳んだ: Gemma 4 MoEの3本RMSNormで統計を共有 (2026-09-04 18:20)

各層の同じhiddenへ掛かる`pre_feedforward_layernorm` 2本とRouter normは、RMSの分母を数学上共有できる。
30層限定・既定off・source hash固定のcoordinatorと、入力を1回読んで3出力を書くMetal kernelを試した。
実モデル128 token、短文3本、off/on/on/offで全30層が発火したが、off **11.454** 対 on
**13.871 ms/token**でonが**+21.1%遅い**。さらに3/3 promptで生成列が分岐した。自前の縮約順が
Routerのexpert選択へ波及し、標準RMSNorm 3回より自前1回の固定費も高い。512 token・強冷却へ進まず、
実装・knob・harnessは残さない。Gemmaレーンは組み込み`QuantizedKVCache`の適用可能性へ進む。
...
- **MoE compile汎用化は棄却**: 実クラスへ配線したABBAは出力完全一致だが、短ms/round -0.6%、
  50k **+0.9%**。50k最初のon armは未空焼きverify幅のtrace税で37.18 ms、以後も取り分約-0.3%に
  留まり、幅別graphの起動・保持コストに見合わない。汎用配線は削除し、Qwen3.6で「40層有効」と
  誤表示していたblock数判定だけqwen4_exp実instanceへ限定した。
- **長文MTP幅を測定中**: 32-token捨て走行の50k MTP +4.7%はアーティファクト。256-token捨て後は
  現行MTPがAR比-0.7%。一方、幅4以下の`cap3`は現行MTP比**-5.3%**、AR比-6.0%（13.236
  ms/token）。受理2.626→2.427よりround費用36.69→32.12 msの削減が勝つ。別走行の25k/32k/
  40k/50kでも現行比-3.6/-7.4/-6.5/-5.0%と全勝。低い側も短文-0.4%（同着）、4k -8.1%、
  8k -2.1%、17k -2.7%、25k -3.6%。0〜50kの全点非退行なので文脈長閾値は置かず、短文/4k/
  17k/50kの非重複3 promptとtemp>0・品質を通してQwen3.6専用既定にするか判断する。
- **非重複promptの再現**: 短文/4k/17kを各3 prompt × 512 tokenで測り、`cap3`は9/9で非退行。
  3 prompt平均は現行比-5.8/-7.8/-11.3%（137.0→145.4、125.2→135.8、103.7→117.0
  tok/s）。50kも3/3で非退行、平均13.477→12.108 ms/token（-10.2%、74.2→82.6 tok/s）。
  greedy速度は全ゲートを通過し、残りはtemp>0と同一継続KLD。
- **品質合格**: temp=0.7の固定AR継続を実verify幅1/4/9/17へ流したtop-4096 KLDは、幅4
  0.003980、現行MTP上限の幅9 0.003936、差**+0.000043**（受け入れ幅+0.0005）。最大裾質量
  0.000768で近似も有効。lookup上限の幅17より幅4が短文/4k/17kの全平均で近い。残りは
  temp=0.7実生成速度、既定配線、formal full。
- **temp=0.7全域適用は棄却、長文条件付きへ**: 3 prompt平均は短文-1.5%、4k -10.4%、17k
  -9.4%だが、短文1件でlookupが4.376 token/roundまで当たり`cap3`が+8.9%悪化した。4k/17kは
  6/6で改善。入力長3,830 token以上だけ`cap3`、未満は現行3/8/16を維持する候補とする。
...
- **次のKV帯域候補**: MLX-LMの`QuantizedKVCache`をfull-attention 10層だけq8/q4にできる。
  ...
  mixed-cache A/Bを速度ではなくメモリ候補として判定する。
...
- **完了**: GuideLLMが送る`ignore_eos`をchat/completionsのstream・non-stream全4経路で
...
## 棄却: Qwen3.6 full-attention KV q8 (2026-09-05 04:46 JST)

full-attention 10層だけをprefill後にmlx-lmの`QuantizedKVCache` q8/group64へ変換し、
GDN cacheはbf16のまま残した。17kの同一process ABBAでは、現行bf16に対して
ms/round **+10.9%**、tok/round **-6.7%**、ms/token **+18.9%**。生成列も19 token目で
分岐した。q8 cacheでは現行の汎用SDPA幅分割を通れず、逆量子化と受理率低下がKV読出し量の
削減を上回る。50kへ進まず製品実装を撤回した。q4は誤差と逆量子化の代償がさらに大きいため、
同じbuilt-in経路では試さない。
...
- K2bだけを実測済みのKV長16,000〜18,000で64 blocksにし、それ以外はMLX表へ戻す。
...
- 追補: 4kはms/token +6.5%、tok/round -6.3%だったため、下限を16,000へ引き上げた。
