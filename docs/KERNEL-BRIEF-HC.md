...
MoE ルーティング 10.1ms と GDN 7.8ms。どちらも同じ「起動回数が多い」型。
lm_head を含む残り 9.9ms もいずれ対象になる。

積み上げの見積もり: hyper-connections で 30 tok/s、MoE と GDN で 45 tok/s、
残りまで畳んで 70 tok/s 圏。理論上限は帯域律速で 125-160 tok/s
(活性 6B x 約 2.5-3.3GB/token を M3 Max の 400GB/s で運ぶ)。
