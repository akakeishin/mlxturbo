...
**1 プロセス 1 モデル。** Ollama や LM Studio のようにサーバー1つで複数モデルを切り替えることはできない。
別のモデルを使いたい場合はプロセスを再起動する (別ポートで2プロセス目を上げれば同時に複数モデルを
サーブすることは可能だが、1プロセスあたりのメモリ消費が大きいモデルを複数常駐させる前提の設計ではない)。
