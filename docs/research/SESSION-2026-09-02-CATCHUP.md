...
### 2026-09-04 18:07 強冷却を常時強化した直後のFlash-Next診断

ユーザーが強冷却を有効にした状態で、現行既定の`null` knobを短文3本 × 512 token、burn-in後の
ABBAで測った。両armは同一設定で、対照出力は3/3一致した。

| 指標 | A | B |
|---|---:|---:|
| ms/token | 17.075 | **16.809** |
| tok/s換算 | 58.6 | **59.5** |
| ms/round | 36.999 | **36.433** |
| tok/round | 2.186 | 2.186 |

prompt別のB平均は57.4 / 54.7 / 67.9 tok/s。直近の通常条件の小ベンチ53.6 tok/s比では約+11%だが、
コード時点と熱履歴が同一でないため冷却だけの厳密差とはしない。同直後の10秒固定GEMM probeは
中央値12.05 TFLOPS (11.53〜12.40)で、過去の十分冷えた約13.05 TFLOPSの92%。強冷却は効いているが、
筐体内部はまだ完全な定常冷却に達していない。後で同じnullベンチを再実行し、冷却継続時間による差を取る。
最大の効用は冷えた上限を大きく超えることより、熱飽和時の約42 tok/sへの崩落を防ぐことと見る。

同じ常時冷却を続けて再測すると、B平均は**16.249 ms/token = 61.5 tok/s**まで上がった
(1回目59.5比+3.4%、既往の冷えた最良62 tok/sにほぼ到達)。prompt別Bは57.3 / 57.2 /
70.9 tok/sで、tok/roundは1回目と同じ2.186。直後の固定GEMMも12.72 TFLOPS
...
### 2026-09-04 23:14 Qwen3.6へのMoE compile汎用化は50k +0.9%で棄却

起動ログはQwen3.6の構造契約に合うMoEを40層と数えていたが、実際に差し替えるのは
qwen4_expの`SparseMoeBlock.__call__`だけだった。実クラスごとに元関数を保持する汎用配線を試し、
強冷却で`MLXTURBO_MOE_COMPILE=1,0`を同一process ABBAした。

| 文脈 | ms/token on / off | ms/round on / off | 差 | 出力 |
|---|---:|---:|---:|---|
| 短3本 × 256 × ABBA 2 | 7.733 / 7.786 | 20.892 / 21.017 | -0.6% round | 3/3完全一致 |
| 50k × 256 × ABBA 2 | 14.462 / 14.335 | 35.944 / 35.628 | **+0.9% round** | 完全一致 |

50kの最初のon armだけ37.18 ms/roundで、その後は35.47〜35.63。32 tokenの空焼きに出なかった
verify幅を最初の本走行でtraceした税がある。定常部だけなら約-0.3%だが、Qwen3.6は幅3〜6と10が
出て幅別×40層のgraphを作る。短の-0.6%と合わせても起動・保持コストに見合わず、汎用配線を棄却。

コードには、qwen4_expの実instanceだけを数えて差し替える修正を残す。これでQwen3.6は偽の
「MoE compile 40層」ログを出さず、Flash-Nextの既存48層経路は維持する。A/Bリセットにも
`disable_moe_block_compile`を追加した。qwen4対象/別クラス対象外とリセットのCPU契約は3件合格。
修正後のFlash-Next実モデルでも48層へ到達し、短文3本の出力は3/3一致、ms/roundはoff比-0.4%で
既存効果を維持した。結果は`bench/results/qwen36-moe-compile-{short,50k}-strongcool-0904.json`と
`bench/results/moe-compile-flash-regression-strongcool-0904.json`（gitignore）。

### 2026-09-04 23:45 Qwen3.6のMTP/AR曲線（50kは捨て走行不足、後段で訂正）

同じprefill snapshotを毎armへ戻し、MTP（n_draft=3、max_draft=8、lookup=16）と真のAR
（n_draft=0、max_draft=0、lookup=0）をMTP→AR→AR→MTPの回文順で2反復した。各armは256 token。

| 実文脈 | MTP / AR ms/token | MTP差 | MTP tok/round |
|---:|---:|---:|---:|
| 16,832 | 10.474 / 12.549 | **-16.5%** | 2.327 |
| 24,833 | 11.052 / 12.822 | **-13.8%** | 2.510 |
| 31,832 | 12.521 / 13.001 | **-3.7%** | 2.349 |
| 39,833 | 12.658 / 13.167 | **-3.9%** | 2.586 |
| 49,832 | 14.642 / 13.979 | **+4.7%** | 2.485 |

50kでも受理利得は2.485 token/round残る一方、先頭MTP armは15.416 ms/token、末尾は14.132で、
32-token捨て走行では幅別trace/確保を焼き切れていなかった。下の256-token捨て走行では現行MTPが
AR比-0.7%へ戻ったため、この節の50k +4.7%は**測定アーティファクトとして訂正**する。曲線の
再現道具と17k〜40kの方向を見る診断値としてだけ残す。結果は
`bench/results/qwen36-ar-mtp-curve-strongcool-0904.json`（gitignore）。

### 2026-09-05 00:09 Qwen3.6の50kは幅4上限で現行MTP比-5.3%

冷却休止後の固定GEMM 12.73 TFLOPS（12.78比-0.4%）。49,832 tokenを一度prefillし、各方式を
256 tokenずつ捨て走行してから、512 tokenを現行MTP→`cap3`→AR→AR→`cap3`→現行MTPの順に測った。
`cap3`はn_draft=3、max_draft=3、lookup=0で、verify幅を4以下へ制限する。

| 方式 | ms/token | ms/round | tok/round | AR比 | 現行MTP比 |
|---|---:|---:|---:|---:|---:|
| 現行MTP | 13.972 | 36.69 | 2.626 | -0.7% | — |
| `cap3` | **13.236** | 32.12 | 2.427 | **-6.0%** | **-5.3%** |
| 真のAR | 14.073 | 14.10 | 1.002 | — | +0.7% |

現行MTPも十分温めればARをわずかに上回る。長文脈の崖はMTP全体の採算逆転ではなく、幅5以上で
汎用SDPA分割が入り、長いKVを複数回読むroundが割高になることが主因。`cap3`は受理を7.6%落とす一方、
round費用を12.5%落として純利益になった。単一promptだけでは既定にせず、25k/32k/40k/50kの
1 prompt曲線で交差範囲を探し、その近傍を非重複3 promptで確認する。結果は
`bench/results/qwen36-mtp-cap3-ar-50k-strongcool-0905.json`（gitignore）。

### 2026-09-05 00:26 Qwen3.6の幅4上限は25k〜50kの全点で現行MTPより速い

10分休止後の固定GEMM 12.75 TFLOPS（12.78比-0.2%）。上と同じ256-token捨て走行、
512-token回文で、同一promptの25k/32k/40k/50k曲線を取った。

| 実文脈 | 現行MTP | `cap3` | 真のAR | `cap3`の現行比 | `cap3` tok/round |
|---:|---:|---:|---:|---:|---:|
| 24,833 | 10.900 | **10.508** | 12.679 | **-3.6%** | 2.393 |
| 31,832 | 12.063 | **11.166** | 12.973 | **-7.4%** | 2.404 |
| 39,833 | 12.682 | **11.854** | 13.373 | **-6.5%** | 2.498 |
| 49,832 | 14.036 | **13.341** | 13.988 | **-5.0%** | 2.427 |

50kの現行MTPは今回はAR比+0.3%でほぼ同着、`cap3`だけAR比-4.6%。幅5以上を切る効果は
25kまで全点で勝ち、単発50kの方向を再現した。次は短文/4k/8k/17k/25kへ下げ、`cap3`が遅くなる
境界を探す。結果は`bench/results/qwen36-mtp-cap3-ar-curve-strongcool-0905.json`（gitignore）。

### 2026-09-05 00:42 Qwen3.6の幅4上限は短文〜25kでも全点非退行

10分休止後の固定GEMM 12.77 TFLOPS（12.78比-0.1%）。同じ1 prompt、256-token捨て走行、
512-token回文で短文/4k/8k/17k/25kへ下げた。

| 実文脈 | 現行MTP | `cap3` | 真のAR | `cap3`の現行比 |
|---:|---:|---:|---:|---:|
| 22 | 7.641 | 7.607 | 12.055 | -0.4%（同着） |
| 3,833 | 8.678 | **7.976** | 12.225 | **-8.1%** |
| 7,833 | 8.956 | **8.770** | 12.463 | **-2.1%** |
| 16,832 | 10.122 | **9.846** | 12.739 | **-2.7%** |
| 24,833 | 10.872 | **10.483** | 12.741 | **-3.6%** |

上の25k〜50k曲線と合わせ、測った0〜50kの全点で非退行、4k〜50kは-2.1〜-8.1%。
文脈長閾値を置く根拠は消え、Qwen3.6だけ`max_draft=3 / lookup=0`を全域で使う候補になった。
ただし単一promptなのでまだ既定にはしない。短文/4k/17k/50kを非重複3 prompt、同じ512 token回文で
再確認し、次にtemp>0と品質を通す。結果は
`bench/results/qwen36-mtp-cap3-ar-lowcurve-strongcool-0905.json`（gitignore）。

### 2026-09-05 00:59 Qwen3.6の幅4上限は非重複3 promptでも短文〜17k全勝

10分休止後の固定GEMMは12.77 TFLOPS（12.78比-0.1%）。短文、約4k、約17kをそれぞれ
非重複3 prompt、各方式256 token捨て走行後の512 token回文で測った。表は3 promptの
ms/token算術平均で、tok/sはその逆数。各prompt単位でも`cap3`は現行MTPを9/9で上回った。

| 指定文脈 | 現行MTP | `cap3` | 現行→`cap3` | tok/s 現行→`cap3` |
|---:|---:|---:|---:|---:|
| 短文 | 7.301 ms | **6.879 ms** | **-5.8%** | 137.0→145.4 |
| 4k | 7.985 ms | **7.364 ms** | **-7.8%** | 125.2→135.8 |
| 17k | 9.641 ms | **8.548 ms** | **-11.3%** | 103.7→117.0 |

単一prompt曲線だけでなく入力を替えても非退行を再現した。長い側ほど必ず単調に伸びるわけではなく、
proposalの当たり方で差は動くが、個別差は短文-0.1〜-9.4%、4k -7.2〜-8.2%、17k
-2.8〜-16.8%だった。次は50kの非重複3 promptを別走行で確認し、温度付き生成と検証幅4/9の
同一継続KLDを通してからQwen3.6専用既定へ入れる。結果は
`bench/results/qwen36-mtp-cap3-ar-3prompt-short-17k-strongcool-0905.json`（gitignore）。

### 2026-09-05 01:15 Qwen3.6の幅4上限は50kの非重複3 promptも全勝

前走から10分休止し、固定GEMM 12.75 TFLOPS（12.78比-0.2%）を確認した。約50kの
非重複3 promptを、各方式256 token捨て走行後の512 token回文で測った。

| case | 現行MTP | `cap3` | 真のAR | `cap3`の現行比 |
|---:|---:|---:|---:|---:|
| 0 | 13.746 ms | **13.082 ms** | 13.972 ms | **-4.8%** |
| 1 | 12.629 ms | **10.469 ms** | 13.999 ms | **-17.1%** |
| 2 | 14.056 ms | **12.772 ms** | 14.114 ms | **-9.1%** |
| 3 prompt平均 | 13.477 ms | **12.108 ms** | 14.028 ms | **-10.2%** |

平均tok/sは現行74.2→`cap3`82.6（+11.3%）。proposal適合度で改善幅は動くが、50kでも3/3で
同符号だった。これでgreedy速度は単一promptの0/4k/8k/17k/25k/32k/40k/50kと、非重複
3 promptの短文/4k/17k/50kをすべて通過した。残る採用条件はtemp=0.7の速度と、同じ固定継続を
検証幅1/4/9へ流すKLDである。結果は
`bench/results/qwen36-mtp-cap3-ar-3prompt-50k-strongcool-0905.json`（gitignore）。

### 2026-09-05 01:41 Qwen3.6 cap3のtemp=0.7幅別KLDは+0.000043で合格

速度だけで既定化しないため、ARがtemp=0.7で作った固定継続を同じprefill snapshotから
teacher-forceし、本体の実verify経路を幅1（AR参照）、4（`cap3`上限）、9（現行MTP上限）、
17（lookup上限）で通した。短文/4k/17kを各3 prompt、各255評価位置。初回top-256は参照裾質量が
最大0.00810で近似精度不足だったため判定せず、10分休止と固定GEMM 12.78 TFLOPSの後、
top-4096で同じ9ケースを取り直した。

| 指定文脈 | 幅4 KLD | 幅9 KLD | 幅4−幅9 | 幅17 KLD |
|---:|---:|---:|---:|---:|
| 短文 | 0.003543 | 0.004282 | -0.000740 | 0.003576 |
| 4k | 0.004196 | 0.003869 | +0.000327 | 0.004245 |
| 17k | 0.004200 | 0.003657 | +0.000543 | 0.005044 |
| 9ケース等重み平均 | **0.003980** | **0.003936** | **+0.000043** | 0.004289 |

全体の現行比ΔKLDは+0.000043で、宣言済み受け入れ幅+0.0005の8.7%。参照top-4096の最大裾質量は
0.000768で0.001未満、近似の妥当性ゲートも通った。幅4のtop-1一致は全体0.9843、case最小
0.9725（診断値であり採用条件ではない）。17kだけ幅9比+0.000543だが、既定線は
`bench/quant_eval.py compare`と同じ全体平均であり、後からcase/context全件という別基準へ変えない。
一方、lookup上限の幅17に対しては3文脈すべて幅4が近い。

道具は`tools/spec_verify_kld.py`。結果は
`bench/results/qwen36-spec-verify-kld-top4096-temp07-strongcool-0905.json`（gitignore）。残る採用条件は
実際のtemp=0.7生成での速度非退行と製品既定配線後の回帰/fullである。

### 2026-09-05 01:58 Qwen3.6 cap3のtemp=0.7は4k以上で全勝、短文全域適用は棄却

10分休止後の固定GEMM 12.75 TFLOPS。temp=0.7、seed固定、EOSを無視して生成長を512 tokenへ
揃え、現行MTP/`cap3`/真のARを短文/4k/17kの非重複3 promptで回文比較した。

| 指定文脈 | 現行MTP | `cap3` | 現行→`cap3` | tok/s 現行→`cap3` | prompt符号 |
|---:|---:|---:|---:|---:|---:|
| 短文 | 7.353 ms | 7.241 ms | -1.5% | 136.0→138.1 | 2勝1敗 |
| 4k | 8.776 ms | **7.862 ms** | **-10.4%** | 113.9→127.2 | **3勝0敗** |
| 17k | 10.043 ms | **9.097 ms** | **-9.4%** | 99.6→109.9 | **3勝0敗** |

短文case 2は現行のlookupが4.376 token/roundまで当たり、`cap3`は3.160に留まって
ms/token **+8.9%**。平均だけなら勝つが、「全prompt非退行」の事前線を破るため、Qwen3.6全域へ
無条件適用する案は棄却する。4kと17kは6/6で-5.5〜-17.0%なので、入力長3,830 token以上だけ
`max_draft=3 / lookup=0`、それ未満は現行3/8/16を維持する条件付き既定候補に切り替える。
3,830は合格した実promptの最小長そのもの。29〜3,829は現行へ倒し、未測定域へ効果を外挿しない。

次は同じtemp=0.7を50kの非重複3 promptで確認する。結果は
`bench/results/qwen36-mtp-cap3-ar-3prompt-temp07-strongcool-0905.json`（gitignore）。

### 2026-09-05 02:14 Qwen3.6 cap3のtemp=0.7は50kも3/3改善

前走から10分休止し、固定GEMM 12.76 TFLOPS（12.78比-0.16%）を確認した。temp=0.7、
seed固定、EOSを無視した512 token回文を、約50kの非重複3 promptで測った。

| case（実文脈） | 現行MTP | `cap3` | 真のAR | `cap3`の現行比 | `cap3`のAR比 |
|---:|---:|---:|---:|---:|---:|
| 0（49,832） | 14.442 ms | **13.057 ms** | 14.388 ms | **-9.6%** | -9.3% |
| 1（49,833） | 14.998 ms | **12.974 ms** | 15.284 ms | **-13.5%** | -15.1% |
| 2（49,830） | 18.417 ms | **17.544 ms** | 15.948 ms | **-4.7%** | +10.0% |
| 3 prompt平均 | 15.952 ms | **14.525 ms** | 15.206 ms | **-8.9%** | -4.5% |

平均tok/sは現行62.7→`cap3`68.8（+9.8%）。現行比は3/3で同符号なので、事前に決めた
全prompt非退行を通過した。case 2では真のARが最速で、現行MTPもAR比+15.5%、`cap3`も+10.0%。
しかし他2 promptでは`cap3`がAR比-9.3/-15.1%であり、文脈長だけの静的AR切替は成立しない。
短文のlookup大当たりを残すため、Qwen3.6＋MTPの未指定時だけ、入力長3,830以上を3/3/0、
...
### 2026-09-05 06:40 旧棄却D3のSAM最小一致長5は50k +2.5%で不採用

過去の「K2後に再計算」条件が満たされたD3 context-SAMについて、
27Bの現行本番経路で`lookup_ngram=5`と既定4を同一process ABBA比較した。
測定専用に`SpecEngine.generate(lookup_ngram=...)`へ直接値を渡し、製品の設定口は追加していない。

| 文脈 | 5 / 4 ms/token | 5 / 4 ms/round | 5 / 4 tok/round | 判定 |
|---:|---:|---:|---:|---|
| 16,832 | 49.768 / 49.820 (-0.1%) | 119.630 / 119.755 (-0.1%) | 2.404 / 2.404 | 同着 |
| 49,832 | 67.330 / 65.674 (**+2.5%**) | 156.696 / 162.439 (-3.5%) | 2.327 / 2.473 (**-5.9%**) | 悪化 |

17kは512 token全一致。50kは5側が43 token目で軌道分岐した。5は誤発火を減らして
roundを安くするが、一致長4の有用なSAM draftも捨てるため、受理率低下が勝った。
旧見積もりはFlash-Nextの生成列と40 ms/roundの費用表に基づき、現行27Bの120〜162 ms/roundには
...
### 2026-09-05 16:04 Fable再監査、100 tok/sを物理上限ベースへ変更

Fable 5.1 xhighをFlash単体、全モデル共通、棄却再審の3本で走らせた。過去棄却から追加で
復活させる案は0件。自作MTPやモデル固有drafter学習はユーザー判断で当面優先しない。

次の学習なし候補はQwen 27B/35B共通のdepth prior較正。`_POS_ACCEPT_PRIOR`は資料値であり、
現行packの実測ではない。`decode_ab_generic`が捨てていた`accept_trace`と`src_hist`を結果へ残し、
既存`--round-trace`のdraft幅と合わせて位置別採択率を出せるようにした。製品経路は変更していない。

100 tok/s一律を必達線から外す。Flash現行artifactの第一目標は、強冷却・同じ短文ベンチで
61.4→65 tok/s持続、17kで53.1→55 tok/s。Qwen 27Bは4k 32.1から同条件比較先43.2への
接近を優先する。公開目標は同一pack・prompt・出力長・sampling・冷却の競合比で決める。

### 2026-09-05 16:26 Qwen 27Bのdepth prior候補は短文-1.6%、prompt退行で棄却

通常冷却に切り替え、直前のGPU走行から10分休止した。開始時の`pmset -g therm`にはthermal / performance
warningが無かった。まず現行経路を凍結3プロンプト×512 tokenで走らせ、MTP roundだけの位置別採択率を
数えると、1〜3番目は0.777 / 0.548 / 0.375だった。資料由来の現行prior 0.70 / 0.11 / 0.02とは
大きく違い、初回計画はdepth 2から3へ変わる条件を満たした。

実測値を入れた候補`{1:.78, 2:.55, 3:.37, 4:.32, 5:.32, 6:.36, 7:.15, 8:.02}`と現行を、
同一processのABBAで比較した。

| 条件 | 現行 ms/token | 候補 ms/token | 差 | 判定 |
|---|---:|---:|---:|---|
| 短文3本平均 | 27.583 | 27.130 | -1.6% | 採用線-2%未達 |
| case 0 | 31.149 | 29.373 | -5.7% | 改善 |
| case 1 | 27.404 | 27.378 | -0.1% | 同着 |
| case 2 | 24.195 | 24.639 | +1.8% | prompt退行線+1%超過 |

全体ではms/round -2.2%だったが、tok/roundも-1.4%。case 0はround数217→208へ減った一方、
case 2は95→112へ増えた。候補はcase 0の27 token目、case 2の298 token目から現行出力と分岐し、
EOSまでの生成長もcase 2で345対381になった。平均速度とprompt別の両採用線を破ったため4k / 17k / KLDへ
進まず棄却した。製品定数は変えず、実験wrapperも削除した。Qwen 35BはMTPとdepth capが異なるため、
通常冷却で十分休ませた後に同じ診断だけを別途行う。

### 2026-09-05 16:39 Flash 65 tok/sは中間ゲート、公開到達点にはしない

ユーザー確認により、短文65 tok/s持続は現行61〜62 tok/sからの中間ゲートに留める。MTPLX #391の
16k 80.92 tok/sとはpack、temperature、固定depth、冷却が異なるため直接の勝敗にはしないが、65で
高速化を打ち切ってもmlx-serve / MTPLXに対して強い訴求にはならない。公開時は同一pack・prompt・出力長・
sampling・冷却で競合を並べ、速度だけなら80 tok/s級への接近を目安にする。100 tok/sは物理根拠のない
stretchのまま残す。強冷却へ切り替える正式測定も、開始前に数分休止し、ユーザーの確認後に始める。

### 2026-09-05 16:47 Qwen 35Bのdepth prior候補は短文+1.5%で棄却

Qwen3.6-35B-A3Bの同一process・凍結3プロンプト×512 tokenで、現行priorと実測位置別率
`{1:.77, 2:.50, 3:.37, 4:.29}`のABBAを通常冷却で比較した。平均は現行7.406 ms/tokenに対し
候補7.516 ms/tokenで+1.5%。ms/roundは-0.3%で同着だったが、tok/roundが2.978→2.884
と-3.2%に下がった。prompt別は-1.1% / -2.5% / +9.9%で、3本目が明確に退行した。

平均-2%以上、各prompt+1%以内の採用線を両方破ったため、4k / 17k / KLDへは進まず棄却した。
製品priorは変えず、実験wrapperも削除した。Qwen 27B/35B共通のprior較正レーンはここで閉じる。

### 2026-09-05 17:31 非MTP FlashのS=1 GDN全段融合を採用

Rapid-MLXのApache-2.0実装を基に、qwen4_expのS=1だけを対象としてdepthwise conv、
q/k正規化、GDN再帰更新、gated RMSNormを1つのMetal dispatchへ融合した。通常冷却・
同一モデル・短文3本×256 tokenのABBAは、現行38.463 tok/sに対して39.555 tok/s
（+2.84%）。条件別も+2.99% / +3.18% / +2.36%で全勝した。

融合あり同士／なし同士の生成列は自己一致したが、両者は19〜39 token目で分岐したため、
速度だけでは採用せずS=1 teacher-forced品質を測った。31課題のKLDは0.01832→0.01843
（Δ+0.00011、受け入れ幅+0.0005以内）、top-1一致率は0.961→0.963だった。
`MLXTURBO_GDN_DECODE_ALL`を既定on、`=0`を退避経路として採用する。

この経路は確定トークン1個の最終状態だけを返す。Flash MTPは複数候補位置の中間状態を
保持して受理位置へrollbackする必要があり、既存`capture()`がGDN入口を置き換えるため
今回のS=1経路を通らない。次は数式とMetal本体を共有しつつ、S>1版だけ中間状態を返す
別契約として実装する。単純な全状態書き出しで帯域を浪費しないことを速度ゲートにする。

### 2026-09-05 17:52 MTP S=2..4 GDN全段融合はround +0.4%で棄却

S=1と同じMetal算術をS=2..4のカーネル内時間ループへ広げ、rollbackに必要な各位置の
fp32状態だけを書き出す試作を行った。既存の前処理融合＋再帰＋gated RMSNormとのmicro比較は、
S=2で+3.4%、主要なS=3で+16.3%、S=4で+15.3%。conv状態は完全一致し、各位置の再帰状態は
最大差約1e-4、bf16出力は最大差0.0078125だった。

しかし通常冷却・同一モデル・短文3本×256 tokenのABBAでは、ms/roundが35.217→35.368
（+0.4%悪化）で、3条件すべて+0.17〜+0.62%だった。tok/roundも2.192→2.141
