...
+1 を直したら、一律 5bit も混在の v-l も走った。向こう自身が
「mlx-serve は config の per-tensor 上書きを読まない」と書いていて、ビット数は
完全にテンソルの形から割り出している。混在に制約は無い。

128 トークン生成、同じプロンプト、M3 Max:

| パック | 常駐 | decode |
|---|---|---|
| 一律 4bit (向こう) | 66.4 GB | 50.4 tok/s |
| v-l (混在) | 72.2 GB | 43.9 tok/s |
| 一律 5bit | 80.4 GB | 44.7 tok/s |

**v-l は 5bit より 8GB 小さいのに遅い。**v-l は GDN 全 36 層と self_attn 全 12 層が
6bit で、Metal ソースに 6bit の速い分岐がほぼ無い。読むバイトは減っても展開が
遅くて食い潰す。

4/5/8 の上では常駐バイトがほぼそのまま速度になる (66.4GB→50.4、80.4GB→44.7)。
**一律 4bit より速くしたければ、4bit を割るしかない。**バイナリには IQ2 の
カーネル (`Metal IQ2/IQ2 streaming decode`) があるので 2bit には経路がある。

向こうは imatrix で重みごとの誤差を測って予算を貪欲配分する仕組み
(`tests/qwen38_iq_allocate.py`) を持っているが、**対象は Qwen3.8-27B で、
Flash-Next 向けには公開していない。**
...
## カーネルが受けるビット幅 (src/transformer.zig, MIT)

経路ごとにバラバラで、ここを外すと「小さいのに遅い」になる。

| 経路 | 受けるビット | 場所 |
|---|---|---|
| MoE 融合 (gate+up / down+reduce / gather-qmv) | **2 / 4 / 8** | 23852, 24016, 24222 |
| hyper-connection 融合 | 2 / 4 / 8 | 23651 |
| GDN blocked | 4 / 5 / 8 | gdnBlockedSource (407-) |
| qmm NAX | 4 / 5 / 6 / 8 | 1863- |
| 一様プロファイル判定 | 4 / 6 / 8 | 6921, 8313 |

MoE は `group_size % (32 / bits) == 0` も要る。2bit なら gs は 16 の倍数。

**エキスパートはパックの 93.7% なので、実質 MoE のゲートが全部を決める。**
2 / 4 / 8 の外を使うと、そこだけ融合カーネルから落ちる。一律 5bit を走らせた
ときに `[moe] fused ...` のログが出なかったのはこれで、v-l が 5bit より
小さいのに遅かったのも、末尾 8 層の 6bit エキスパートが落ちていたため。

### 一律 4bit より速くする道

**2bit を使う。**カーネルは既にある。

| | 実効ビット | エキスパート | MoE 速い経路 |
|---|---|---|---|
| 一律 4bit gs64 | 4.5 | 63.3 GiB | ○ |
| 2bit gs64 | 2.5 | 35.2 GiB | ○ |
| 3bit gs64 | 3.5 | 49.2 GiB | ✕ |
| 5bit gs64 | 5.5 | 77.4 GiB | ✕ |

混在は通るので、効かない層だけ 2bit に落として効く層を 4bit で残せる。
2 と 4 はどちらも速い経路の上にある。

3bit を使いたいなら `src/transformer.zig` の unpack テンプレートに分岐を足す
ことになる。中身はビット幅ぶんシフトして取り出すだけで難しくないが、Zig の
ビルドが要る。**先に 2bit を試して、足りなければ考える。**
