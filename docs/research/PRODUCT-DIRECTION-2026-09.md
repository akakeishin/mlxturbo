...
- 「特定モデルの decode の数 % 最適化を追わない」は半分だけ。**小 M qmm は全族の verify 幅に効く共通項で「同じ重みで負けない」に要る → 仕上げる。**Flash-Next の 1〜2% の的 (飛ばす / 積む、層単位 compile) も**プロトタイプとして試す価値はある** (ユーザー 11:41: 他に転用できる部分がある)。ただし P0 と並走で、P0 を止めない。
