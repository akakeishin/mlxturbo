...
**公式レップが 5 回続けて無効になった真因は 65W 充電器だった。**
`bench/results/compare-official2-rep2-invalid-65w.json`。5 回目は排他窓を
確保して他プロセスをゼロにしたのに、4 回目より悪化した (0.50 → 0.33)。
そこで電源側を見たら **`ioreg` の `"Watts"=65`**。この機体は Mac15,9
(MacBook Pro 16" M3 Max / 128GB) で、付属は **140W**。

決め手は負荷中の電流。`InstantAmperage` を 30 秒サンプリングして
**ずっと 0**。バッテリーは充電も放電もしていない = システムが 65W の枠に
収まるよう SoC の電力を絞っている (満充電時、macOS はバッテリーを削るより
性能を落とす方を選ぶ)。16" M3 Max は GPU 連続負荷だけで 60-90W 使うので、
表示・SSD・USB バスパワーを足すと 65W では全く足りない。

旧 rep3 との比 (5 回目):

| ラン | 健全 | 今回 | 比 |
|---|---|---|---|
| 1 mlx-lm sq code | 23.16 | 22.87 | 0.99 |
| 2 mlx-lm sq prose | 23.15 | 19.49 | 0.84 |
| 3 mlx-lm sq edit | 22.93 | 9.28 | 0.40 |
| 4-18 (全エンジン) | — | — | **0.24-0.36 の台地** |

観測された形が全部これで説明できる。ラン 1 が満点なのは PD アダプタが短時間
なら定格超を出せるため。ラン 3 で崩れて以後平らなのは、持続要求が定格を
超えてキャップが噛むため。サーマル警告が出ないのは熱ではなく電力予算だから。
アイドル後の単発計測が 23.0 に戻るのは、25 秒 + 間隔なら予算内に収まるため
(事前検証が毎回通ってしまったのはこれが理由。**単発の事前検証では 18 ラン
連続の持続負荷を検出できない**)。旧 rep1/2/3 が 18 ラン通して 23 を維持した
のは、当時は正規の充電器だったということ。

**過去 4 回の判定のうち 2 回は誤り。** 2 回目を「低電力モード」、3 回目を
「持続帯域の選択的スロットリング」、4 回目を「他セッションとの共有」と
書いたが、少なくとも 3・4 回目は本件が主因。powermode 2 だった 2 回目は
両方が効いていた可能性が高い。rsync は 4 回目に実在したが従犯。

**次レップの必須条件に追加 (最優先):**
0. **`ioreg -rn AppleSmartBattery | grep Watts` が 140 (最低でも 96) であること。**
   外付けバスパワー SSD は計測中は外す (同じ枠から引く)。

**公式レップ 4 回目 (再起動後) も無効。当初は他セッションとのマシン共有と判定
した (下記) が、上記のとおり 65W 充電器が主因。rsync は従犯。**
