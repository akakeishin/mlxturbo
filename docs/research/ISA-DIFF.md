...
(→ 同日、ユーザー承認の上で bench/test_dispatch.py の候補を fast_qmm に
揃えた。較正の実施は §9.3)。

### 9.3 経路表の再較正と反転 (2026-08-27、クローラ停止後)

クローラ停止後に依存チェーン較正を 2 ラン実施した
(bench/results/calib-chain-a/b.json、16 ステップ連鎖、4 形状 x M=6..16、
候補 stock / nocap(M<=12) / fast_qmm)。採用規則: 両ランで勝者が一致し、
かつ勝者が現経路を両ランとも 5% 超上回る行のみ反転。判定量 (現経路比) は
ラン内比なので、バックグラウンド負荷 (dasd) による絶対時刻のドリフトは
相殺される。

結果: 44 行中 27 行が反転し、fast_qmm (MMA) が M=6..16 のほぼ全域を取った。
勝ち幅は nocap からの奪取で 1.06-1.38x、stock からの奪取 (M=12..15) で
1.38-1.59x。残る nocap は (17408,5120) M=9 の 1 行のみ (2 ランで勝者不一致の
ため基本形を維持)。単発レイテンシの勝者 (single_best) も全行で chain と
一致した。M=6/7 が nocap に負けていた旧較正は、ベクトル化前の fast_qmm と
v5 skinny 候補のズレの上に成り立っていた値で、どちらも解消済み。

受け入れ: bench/test_dispatch.py 45 形状の数値ゲート違反 0 (MMA 行 8e-3 帯)、
bench/test_dispatch_static.py 全通過 (経路表アサートは新表に追従)、
bench/gate.py の baseline_all_identical は同日実施のランで確認する。
タイミングバー (dispatch_over_best) は dasd バーストによる順序バイアスで
同一カーネル同士でも 1.1-1.4x に振れたため、較正主張には使っていない
(交互計測では wrapper 差は ~1us、時間差ゼロに収束することを確認済み)。
