...
## バッチ x 投機の紙モデル (2026-09-01 夜、advisor 指定の投資判断表)

8 プロンプト独立走行の router 選択から、同一ラウンド・同一層の expert
和集合を計数 (union_curve、計数のみなのでダウンロード並走でも有効)。
3 項モデル round = 密部固定 26.7ms + MoE 読み + KV 0.5ms/seq を B=1/P=3 の
実測 (round 41ms / MoE 13.8ms / tok_step 1.98、4k) で較正。stock は行ごと
重み読み (gather_qmv、共有なしはディスパッチ特定済み)、shared は共有タイル
カーネルの到達点 (union ごと読み)。B=1/P=3 の予測 48.3 vs 実測 48.8 で整合。

| B | P=T+1 | stock 合計 tok/s (倍率) | shared 合計 tok/s (倍率) |
|---|---|---|---|
| 2 | 3 | 71.6 (1.48x) | 76.4 (1.58x) |
| 4 | 3 | 94.4 (1.95x) | 114.1 (2.36x) |
| 8 | 3 | 112.3 (2.32x) | 161.5 (3.34x) |
| 8 | 2 | 126.6 (2.62x) | 155.6 (3.22x) |

決まったこと 3 つ:
1. **レーン続行** — 反転条件 (B=4 で 1.6x 未満なら畳む) は発火せず。B=8 +
   共有タイルで 3.3x はアップグレード帯 (2.5x 超)。
2. **decode 共有タイル①の凍結解除、ただし設計点はバッチ** — 取り分は B で
   成長する (B=1 で互角 → B=4 で +21% → B=8 で +44%)。v2 のセグメント設計を
   B(T+1) 行に一般化する形で書く。B=1 単独の micro で再び判定しないこと。
3. **depth コントローラの向き** — stock では B=8 で P2 > P3 に反転 (advisor の
   予言どおり浅くなる)。共有タイルが入れば P3 が B=8 でも持つ。負荷連動
   depth (depth=0 含む) は admission 側に置く。

モデルの限界 (正直な注記): 位置ごとの indexer/attention 費用を密部固定に
畳んでいるので P=4 列は楽観的 (実測の depth3 劣化を再現しない)。KV 項は
4k 較正の粗い定数。投資判断と符号の決定にのみ使い、絶対値は実装後の実測で。

### バッチ計測の判定プロトコル (実測前の宣言)

- バッチ経路は QSA の causal 化を含み**設計上ビット一致しない**。判定は
  複数プロンプト x 固定シード x tok/step 平均 + KLD (受け入れ幅 +0.0005) に移す。
- **B=1 無劣化ゲート**: バッチ機構を有効化しても単一リクエストの短 decode
  14.6ms/tok が動かないこと。これを bench の回帰に固定してから機構に着手。
...
## QSA 因果性修正の A/B (2026-09-01 夜、実測)

端数ブロックの未来漏れを塞いだ (`5d1e1c5`) 影響を、1 プロセス内 ABBA 交互で
測った (`tools/qsa_causal_ab.py`、結果 `bench/results/qsa-causal-ab.json`)。
A = 修正後、B = 修正前 (返り値の端数列を True に戻す薄い包みで再現。写しは
作っていない)。全条件 512 トークン、長文脈は実文書から切った 16.9k を 3 本。

**対照 (短文脈、QSA 不活性)**: A と B で出力トークン列も tok/round も完全一致。
ms/tok の差は 0.01-0.07ms (0.2% 未満)。修正が触れない領域は触れていない。

**本番 (16.9k、QSA 活性)**:

| 指標 | A (修正後) | B (修正前) | 差 |
|---|---|---|---|
| tok/round | **1.972** | 1.932 | **+2.1%** |
| ms/token | 33.4-35.1 | 34.1-34.1 | 一貫した符号なし |

tok/round はプロンプトごとに +6.5% / -1.5% / +1.6% で、平均 +2.1%。同一条件の
繰り返しは 3 桁目まで完全再現する (貪欲なので当然) が、**プロンプト 3 本では
符号が揃わない**。「受理率を落とさない」までは言えるが、「上げる」は主張しない。
ms/token は一貫した符号が出ず、実質的に相殺。**修正の代償はほぼゼロ**。

**測定の作法として拾ったもの**: プロセスの最初の 1 本は捨てること。最初の
short A だけ prefill 41s / 19.19 ms/tok と出て (以降 0.24s / 16.45ms)、これを
含めると「long ms/tok -4.94%」という嘘の差が出る。温まってからの繰り返しは
0.2% 以内に収まるので、この harness の分解能はそこまで細かい。

**prefill の 44-45s は熱だった (追試で決着)**: 同じ 16.9k プロンプトで
TTFT だけを繰り返し測ると、冷たい 1 本目 73-77s (重みのページイン込み、捨てる)
のあと 35.3-35.9s で落ち着く。A/B harness の 44-45s は 15 分回し続けた後の
値で、熱で 25% 落ちていた。**長時間の連続計測では 25% 落ちる**ので、
絶対値を取るなら冷ましてから短く測ること。

**リファクタ (段 1-5) は prefill を遅くしていない**。同じ手順・同じ
プロンプトで mlxturbo/ だけ入れ替えて測った:

| | rep1 | rep2 | rep3 | 平均 |
|---|---|---|---|---|
| 段 1-5 前 (1c41ede) | 34.57 | 35.47 | 36.79 | 35.61s |
| 段 1-5 後 (HEAD) | 35.51 | 35.34 | 35.85 | **35.57s** |

差 0.1% で、rep 間のばらつき (±3%) に埋もれる。シームの分だけ Python の
呼び出しが層あたり 3-4 個増えたが、prefill は 2048 幅なので効かない。

なお 35.6s はスコアボードの 32.4-32.9s と合わないが、**両バージョンで同じ値**
なので原因はプロンプトか harness の側 (engine 直叩きで `build_runner` を
通していない)。スコアボードの値を更新する前に、出荷経路で測り直すこと。

## 共有タイル gather v2 の in-model A/B (2026-09-01 夜、決着: 負け)

`MLXTURBO_MOE_VERIFY` (kernels/moe_verify_gather.py の v2、gate+up 融合 +
down) を出荷経路の decode に入れて 1 プロセス内 ABBA で測った
(`tools/decode_ab.py --knob moe-verify`)。A = kernel on、B = 既定 off。

| プロンプト | A (v2 on) | B (off) | 差 |
|---|---|---|---|
| short ctx=62 | 24.92 / 24.96 | 16.41 / 16.40 | **+52% 遅い** |
| short ctx=64 | 27.82 / 27.79 | 18.54 / 18.55 | **+50% 遅い** |
| short ctx=68 | 22.87 / 22.89 | 15.64 / 15.74 | **+46% 遅い** |

同一条件の繰り返しは 0.1% 以内で再現。3 プロンプトとも符号が揃い、幅が
桁違いなので**長文脈を測る前に打ち切った** (verify 幅の MoE 呼び出しは
文脈長に依らず同じで、17k では round の他の部分が伸びるぶん相対ペナルティが
縮むだけ。結論は動かない)。

**判定: 既定 off のまま据え置き。**micro で勝って in-model で負ける前例
(moe_glu v1-v3、moe_route、HC prefill 融合) にもう 1 つ加わった。
fp32 参照との allclose は通っているので正しさの問題ではなく、単に遅い。
このカーネルを生かすなら B(T+1) 行のバッチ版から作り直す話になる
(バッチ設計点が決まってから)。

**「共有タイル入りで 114.1 tok/s」という紙モデルの前提はここで崩れた。**
バッチの見積もりから共有タイル項を外すこと (B=4 で 94.4 tok/s、
B=8 で 1.95x 側の系列だけを残す)。
