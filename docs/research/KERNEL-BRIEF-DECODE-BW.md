...
**未解決 (次に見る)**: この harness の 16.9k prefill は 44-45s で、
スコアボードの 32.6s と合わない。候補は (a) `runner.build_runner` を通して
いない (wired limit 未設定)、(b) 実文書プロンプトは n-gram の行が散るので
ディスクを引く、(c) 熱。**engine 直叩きの絶対値をスコアボードに混ぜないこと。**
prefill の現在地は出荷経路で測り直す。
