...
## depth 再掃引で既定が変わった (2026-09-01 夜、実測)

`tools/decode_ab.py --knob depth` で 1/2/3 を回文順 (1,2,3,3,2,1) に回した。
各条件 512 トークン、文脈ごとに 3 プロンプト。結果 `bench/results/depth-*.json`。

**ms/token (depth 2 を基準にした差)**:

| 文脈 | depth 1 | depth 2 | depth 3 |
|---|---|---|---|
| 65 tok | +5.6% | 基準 (17.97) | +11.7% |
| 900 | +5.1% | 基準 (19.35) | +2.6% |
| 2.6k | **-3.3%** | 基準 (24.09) | +9.9% |
| 4k | **-3.1%** | 基準 (25.46) | +8.2% |
| 17k | **-10.9%** | 基準 (35.68) | +3.2% |

長文では 3 プロンプトとも符号が揃う (17k で 33.46/30.57/31.36 vs
35.98/36.25/34.80)。**tok/round は深いほど上がり続ける** (17k で
1.64 / 1.97 / 2.33) のに壁時計は逆、というのがこの現象の形。受理が増えても、
検証フォワードに 1 位置足す費用がそれを償却しない。

**反転は 900 と 2.6k の間**で、そこには QSA が働き始める境界
(`indexer_budget` = 2048) がある。機構としても符合する — QSA が活性だと
1 位置あたりにブロック選択と疎マスクの費用が乗る。

**既定を変更**: `DEPTH_CONTEXT_LIMIT` はモデルの `indexer_budget` を既定に
した (`spec_flash._depth_ctx_limit`、`MLXTURBO_DEPTH_CTX_LIMIT` で上書き可)。
これまでは 262144 = 実質「切り替えない」で、17k で depth 2 を使い続けていた。
**長文脈の decode がそのぶん速くなる (17k で -10.9%)。**

前の版の記録「17k 実測は depth2 が depth1 を 14% 上回り」は単一プロンプトの
測定で、複数プロンプト x 512 の平均を取ると符号が逆だった。CLAUDE.md の
「tok/step の比較は複数プロンプト x 512 の平均でだけ行う」がまさにこの罠。

## バッチ x 投機の判定 (2026-09-01 夜、反転条件が発火 → 畳む)

宣言していた反転条件は **「B=4 で 1.6 倍未満ならレーンを畳む」**
(紙モデルの予測は 1.95 倍。共有タイルの項は同日の実測で外した)。

engine 直叩きの実測 (`bench/batch_spec_throughput.py`、短〜中尺 8 本、
生成 128 トークン、solo は同じプロンプトを 1 本ずつ回した合計):
...
初回に B=1 が 1.0 を割ったのは原因が特定できた -- バッチ側だけ段階投入
(`_staged_forward`) を通っておらず、単独が持っている取り分を捨てていた。
直して測り直したが **B=4 は 1.47 倍で、宣言した 1.6 倍に届かない。**
原因を特定した 1 回きりの修正として測り直す、と決めていたので、ここで畳む。

**畳むが、消さない。**`mlxturbo/batch_spec.py` の生成器は正しく動く
(出力が 1 本ずつの貪欲デコードと完全一致することを `tools/verify_batch_spec.py`
が常設で確認する)。既定では誰も呼ばない。moe_glu v1-v3 と同じ「負の結果の
記録」として残す。

**再訪の条件** (この判定が反転する条件):

- decode 側の帯域効率が上がったとき。いま MoE 50% / attention 25% で、
  バッチの利得は「重みの読み出しを B 行で分け合う」ことから来る。素の効率が
  上がるほどバッチの相対利得は**縮む**ので、これは再訪の理由にならない
- **逆に、ディスパッチ律速の度合いが上がったとき**は再訪する価値がある
  (バッチは 1 ラウンドの起動回数を B 行で割る)。段 6 で段階投入の取り分が
  9.18ms (50k・幅 2 で壁時計の 16.5%) と出たので、この方向は実在する
- サーバー配線まで書けば B=8 の 1.66 倍が実利になる場面 (同時 8 リクエスト)
  はあるが、**それは待ち行列のある運用の話**で、単独レイテンシの土俵ではない。
  レーンの目的は後者だったので畳む判断は変わらない

## 現在地の置き場 (2026-09-04 03:26)

このファイルの末尾を「現在地」にしていたが、9/2 以降の実測と判定は `docs/research/SESSION-2026-09-02-CATCHUP.md` (時刻順の全記録) と
`docs/NEXT-SESSION-PROMPT.md` (現在地と段取り) に移った。着手前に読むのはその 2 つ。9/4 深夜の時点: 既定に入ったのは
n-gram 先読み (on + 投入位置の前倒し)、HC の細長 GEMM の qmm_wide、MoE decode 幅の fused:1。decode の糊の融合レーンは閉じた
(勝ち: GDN 融合 -2.4%、fused:1 -1.3%。行列積は 430 GB/s でピーク、dispatch の本数の値段は稼働率 96% で実質ゼロ)。
prefill の残りは K1 arm A (スコアを実体化しない attention)、その後は 27B レーン。
