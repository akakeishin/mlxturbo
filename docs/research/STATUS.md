...
## 固定費と読み出し量を分ける (2026-08-27)

`tools/ablate.py` は「どの部品が時間を食うか」までしか出せない。その時間が
カーネル起動の回数から来るのか、重みの読み出し量から来るのかを分けられない。
融合で削るのかビットで削るのかは、そこを分けないと決まらない。

分ける道具を 2 つ足した。

- `tools/byte_budget.py` — safetensors のヘッダだけから 1 トークンあたりの
  読み出し量を数える。エキスパートは top_k/512 の割引を入れる。重みは読まない
- `tools/micro_moe_gdn.py` — 本体 98GB を読まずに MoE と GDN を同じ形で作り、
  融合案とビット掃引を比べる。1 層ぶんの乱数重みで足りる

### 読み出しの内訳 (v-stream、1 トークン 6.462 GB)

| | 読む GB | 割合 | 格納 GB |
|---|---|---|---|
| **GDN 投影/conv** | 2.218 | **34.3%** | 2.2 |
| MoE experts (top_k/512) | 1.819 | 28.1% | 93.1 |
| hyper-connections | 0.682 | 10.5% | 0.7 |
| lm_head | 0.675 | 10.5% | 0.7 |
| full attention (12 層) | 0.635 | 9.8% | 0.6 |

格納 93.1GB の experts が読み出しでは 28% しか占めず、格納 2.2GB の GDN 投影が
最大手になる。**格納量で優先順位を付けると外す。**

帯域だけの下限は 16.8 ms/token (60 tok/s)。実測 51.5ms なので帯域の 33% しか
使っていない。

### 3 つの部品は別種の問題だった

ablate の実測を上の帯域下限と突き合わせる。

| 部品 | ablate | 帯域下限 | 差 = 固定費 | 融合の実測 |
|---|---|---|---|---|
| **hyper-connections** | 19.9ms | 1.8ms | **18.1ms** | (カーネルセッション) |
| MoE ルーティング | 10.1ms | 5.1ms | 5.0ms | 0.31ms |
| GDN | 7.8ms | 5.8ms | 2.0ms | 0.59ms |

hyper-connections だけがほぼ全部固定費で、読み出しは 1.8ms しかない。
**融合が効くのはここだけ。**

MoE の融合案は実装して測った。ルータ頭の `mx.compile` -0.14ms、否定の省略
-0.14ms、共有エキスパートを switch の第 512 番に畳む -0.31ms。共有を畳む式は
厳密に一致する (相対誤差 0.00、`check_merged_shared_algebra`) が、畳んでも
共有の重みは読むので、消える固定費のぶんしか出ない。GDN も同様で、投影 4 本を
1 本にまとめる案と定数を rms_norm に畳む案で合計 0.59ms。

「上限」との差が大きいのは、上限が部品を丸ごと消した値で、読み出しごと消えて
いるため。融合は読み出しを減らさない。

### 限界コストは帯域そのもの

MoE のビットを振ると、増分は 385 GB/s で走る。M3 Max の実効ピークに等しい。
**削ったバイトはそのまま時間になる。**

    2bit  8.23ms / 0.59GB      6bit  11.20ms / 1.77GB
    4bit  9.09ms / 1.18GB      8bit  12.82ms / 2.36GB

GDN 投影も同じで、8bit から 4bit で 8.60 -> 4.77ms。

### GDN 投影 8bit -> 4bit で -3.27 ms/token

同一プロセス内の A/B (`tools/rebit_ab.py`)。別ランどうしを比べると熱と電力の
差が結果に化けるので、1 回のロードで前後を測る。

    打ち直し前  51.16 ms/token (19.55 tok/s)
    打ち直し後  47.89 ms/token (20.88 tok/s)

カーネルを 1 行も書かずに、GDN の融合案全部の 5 倍以上。GDN 投影は格納 2.2GB と
小さいので `default: 8bit` に埋もれていた。

### 焼かずにビットを試す

内蔵の空きが少なく、1 本 92GB の焼きを気軽に増やせない。`fastmlx/rebit.py` は
読み込み済みモデルの一部をその場で打ち直す。8bit -> 逆量子化 -> 4bit の二重
量子化なので**本番の焼きより必ず悪く出る**。「これで保てるなら焼いても保てる」
という向きの判定にしか使わない。逆は言えない。

`bench/quant_eval.py sweep` は 1 回のロードでビット構成を積み上げ、段ごとに
品質と速度を同時に測る。段ごとの **ms/KLD** (1 KLD あたり何 ms 買えたか) が
出るので、どのクラスから削るかが folklore でなく交換レートで決まる。

レシピの `default` も `gdn` / `attn` / `hc` / `head` / `shared` に分けた。
書かれていないクラスは `default` に落ちるので既存レシピは無変更。

### 測定の制約

98GB のモデルは**このマシンで同時に 1 本まで**。カーネルセッションと親が
両方読むとメモリ圧で片方が落ちる (実際に sweep が 1 回落ちた)。大物を回す前に
`ps` で相手を確認すること。マイクロベンチ (1.3GB) は並行して構わない。

## v-fast6 の確定値と、融合の効きの食い違い (2026-08-27)

### ビットを掃引で決めた

`bench/quant_eval.py sweep` で 1 段ずつ代償を測り、v-fast6 のビットを決めた
(base = v-stream, KLD 0.00260 から):

| 段 | KLD 増 | 読み出し | 判断 |
|---|---|---|---|
| gdn 8->6 | +0.00063 | -0.554 GB | 採る。一番おいしい |
| head 8->6 | +0.00041 | -0.169 GB | 採る |
| attn 8->6 | +0.00082 | -0.159 GB | 採る |
| shared 8->6 | -0.00015 | -0.063 GB | 採る (誤差。感度が無い) |
| gdn 8->4 | +0.00663 | -1.109 GB | **採らない**。6bit の 10 倍の代償で速度は 2 倍 |
| hc 8->4 | +0.01337 | -0.341 GB | **採らない**。他の全部を足したより高い |

hc は融合カーネルが 4/8 しか受けないので 6bit を選べず、4bit は高すぎるので
8bit 据え置き。カーネルセッションが申し送りで判断を求めていた件はこれで決着。

この結果を v-96 / v-64 にも適用した。両者は `default` を一括で 4bit に
落としていたので hc も gdn も 4bit だった。hc は格納 0.7GB しかなく、
そこを削っても容量では何も得られない。戻して +0.7-1.0GB (v-96 71.7GB、
v-64 49.0GB)。

### rebit の予測は 14% 悲観側だった

v-fast6 の rebit 予測 KLD 0.00432 に対し、実際に焼いたら **0.00378**。
二重量子化ぶんが悲観側に出るという見立てが実測で裏付けられた。**rebit は
「これで保てるなら焼いても保てる」の判定に使える**。逆は言えないままだが、
幅がおよそ 15% と分かったので、境界付近の判断にも使える。

### 確定値 (v-stream との比較、同一の道具・交互測定)

| | 融合なし | 融合あり | 差 |
|---|---|---|---|
| v-stream (6.262 bpw, 92GB) | 51.99 ms | 41.38 ms | -10.6 |
| **v-fast6 (6.206 bpw, 91GB)** | **49.52 ms** | **37.84 ms** | **-11.5** |

- 読み出し 6.462 -> 5.517 GB/token
- KLD 0.00260 -> 0.00378、top1 0.9881 -> 0.9850
- 融合なしの差 -2.47ms は、バイト予算の予測 -2.45ms とほぼ完全に一致した。
  **削ったバイトはそのまま時間になる、が実測で確認できた**

到達点は **37.84 ms/token = 26.4 tok/s**。最低ラインの 30 tok/s には届いていない。
...
- **融合なしの基準は一致する** (52.32 対 51.99)。つまりマシンの状態や
  熱の違いでは説明できない。基準が同じで融合後だけ 6ms 違う
- 測り方もほぼ同じ (どちらも 1 トークンごとに `int(mx.argmax(...))` で同期する
  逐次デコード)。違いは n=25 対 n=60 と、温めの長さだけ
- こちらは温めを 1 パス入れた上で 3 回の最小値を取り、さらに融合を on/off
  交互に測って裏を取っている (41.38 と 42.14 で一致)

**未解決。**どちらの数字を公表値にするかが決まらないので、カーネルセッション側で
n=60・温めあり・交互測定で取り直してもらう必要がある。

### 残りの内訳 (v-fast6 + 融合、37.84ms)

| | ms | 種類 |
|---|---|---|
| 帯域の下限 (5.517GB / 385GB/s) | 14.3 | 削るならビット |
| MoE の固定費 | 5.9 | 融合しても 0.3ms |
| hyper-connections (融合後) | 4.8 | ほぼ底 |
| GDN の固定費 | 2.2 | 融合しても 0.6ms |
| **残り (embed/lm_head/norm/dispatch)** | **~10** | **未分解** |

**次に見るべきは「残り」の約 10ms。**いまや最大の未調査ブロックで、
hyper-connections を畳んだ後では最大の固定費でもある。ablate.py は
この塊を分解していない。

### 「残り」を分解した (2026-08-27)

`tools/rest_profile.py` で ablate.py の続きを外した (v-fast6 + 融合カーネル、
このときの全体は 33.91ms):

| 部品 | ms | 帯域下限 | 差 = 固定費 |
|---|---|---|---|
| MoE ルーティング | 11.01 | 5.5 | 5.5 |
| GDN (44 層) | 6.76 | 4.3 | 2.4 |
| **lm_head** | **4.89** | **1.3** | **3.6** |
| hyper-connections (融合後) | 4.51 | 1.8 | 2.7 |
| PLE (n-gram) | 2.87 | — | |
| full attention (12 層) | 2.53 | 1.2 | 1.3 |
| 残差の合成 (96 回) | 0.30 | | |
| QSA indexer | 0.07 | | 短文脈では走らない |
| ここまで外して残る量 | 0.98 | | |

ablate.py が「残り 9.9ms」としていた塊の正体は **lm_head 4.89ms +
full attention 2.53ms + 残差 0.30ms + 端数**だった。未知の起動コストではない。

### 単体ベンチは達成可能な帯域を過大に見せる

lm_head (2560 -> 248320) を単体で測ると:

| bits | 読む GB | ms | 実効 GB/s |
|---|---|---|---|
| 4 | 0.318 | 1.45 | 220 |
| 6 | 0.477 | 1.87 | 256 |
| 8 | 0.636 | 2.27 | 281 |
...
単体なら 6bit で 1.87ms。**モデルの中では 4.89ms** かかる。差の 3ms は他の
部品と帯域を奪い合うぶん。micro_moe_gdn.py の実効帯域 (MoE 385 GB/s) も
同じ理由で楽観側に出ている数字なので、**マイクロベンチは案の優劣を比べる
用途に限り、絶対値の予測に使わない**こと。

なお bf16 が 4-8bit より遅い (180 GB/s) のは、読む byte 数が倍だから。
量子化は品質を落とす代わりに速度を買う取引だが、lm_head では 8bit でも
bf16 の 3 倍速い。

### 帯域待ちを埋める 3 つの形

「読み込み待ちの間に何かできないか」への答え。

1. **カーネルの中** — GPU がスレッドを増やして自動で埋める領域。埋まって
   いないなら (lm_head の 104 GB/s のように) カーネルの作りの問題
2. **トークンをまたぐ = MTP / 投機デコード** — 一括 forward は S=16 でも
   S=1 の 1.18 倍。重みを 1 回流す間に 16 トークンぶん計算できる。
   受理されたドラフトはほぼタダ
3. **リクエストをまたぐ = バッチ** — 同じ重みの流れで複数の会話を処理する。
   ローカルの単独利用では効かないが、サーバとして出すなら同じ理屈

1 で取れる上限は MoE 5.5 + GDN 2.4 + lm_head 3.6 で、33.91 -> 23ms
(43 tok/s) 圏。2 はそれと掛け算になる。

### 訂正: 一括 forward のスケーリングを測り違えていた (2026-08-28)

`tools/decode_profile.py` の `bench_batch` が、プロンプトの forward を eval せずに
タイマーを開始していた。MLX は遅延評価なので、次の eval がプロンプトぶんまで
巻き込んで計測区間に入っていた (S=1 が 490ms と出ていたのが兆候。逐次デコードは
50ms/token なので辻褄が合わない)。

直して測り直した値 (v-fast6、融合なし、逐次は 50.0ms/token):

| S | ms/forward | ms/token | S=1 比 |
|---|---|---|---|
| 1 | 56.45 | 56.45 | 1.00x |
| 4 | 65.95 | 16.49 | 1.17x |
| 8 | 96.90 | 12.11 | 1.72x |
| 16 | 143.01 | 8.94 | **2.53x** |

**以前の「S=16 でも S=1 の 1.17 倍、限界コスト 2.9ms/token」は誤り。**
正しくは 2.53 倍で、限界トークンは **5.77ms**。

理由: **512 エキスパートの細粒度 MoE では、S を増やすと活性化するエキスパートが
増える。**1 トークンなら 10 個だが、S トークンでは独立と仮定して
512 x (1 - (1 - 10/512)^S) 個 — S=16 なら約 138 個で、エキスパートの読み出しが
約 14 倍になる。追加ぶん 86.6ms を 385 GB/s で換算すると 33GB で、
experts 1.819GB x 12.8 = 23GB と桁が合う。

密なモデルなら一括 forward は本当にほぼタダになるが、**この構造では
S に対しておおよそ線形に高くなる。**

#### 何が変わって、何が変わらないか

- **変わらない**: hyper-connections がディスパッチ律速だという判断。これは
  S スケーリングではなく「読み出し 0.682GB (=1.8ms) に対し実測 20.9ms」という
  独立した根拠で出しており、融合カーネルが実際に 20.9 -> 4.51ms にしたことで
  裏付けられている
- **変わる**: MTP の見込み。「受理されたドラフトはほぼタダ」ではない。
  深さ 4 (S=5) なら 1 ステップ約 79.5ms で、全部受理されて 15.9ms/token
  (3.1 倍)、現実的な受理率なら 2 倍前後。それでも大きいが、**深さは浅く
  保つべき** (エキスパートの読み出しが S にほぼ比例するため)
- **変わる**: バッチの経済性。S=16 で 8.94ms/token = 逐次の 5.6 倍の
  スループット。サーバとして出すなら効くが、こちらも青天井ではない
...
|---|---|---|---|---|---|
| 朝の出発点 (v-stream) | 52.32 | 19.1 | 92GB | 0.00260 | 0.9881 |
| **v-fast6 + 融合カーネル** | **32.67** | **30.61** | 91GB | 0.00378 | 0.9850 |
| v-96 + 融合カーネル | 29.59 | 33.79 | 67GiB | 0.02902 | 0.9519 |

**MTP 無しで 30 tok/s を超えた。**内訳は hyper-connections の融合カーネル
(-14.6ms、カーネルセッション) と、読み出しを減らすビット配分 (-2.5ms)。
この表の数字は同期ループ (tools/bench_fused.py) で測ったもので、出荷経路の
数字は下の「合成値」で別に取った。

### 合成値 (出荷経路で確定)

**v-fast6 + 融合 + pread = 32.34 ms/token = 30.92 tok/s、非投機・build_runner 経路。**

n=9 の中央値で IQR 32.29-32.38。条件は `bench/results/composed-shipping.json`
にある (融合 HC のみ有効、moe_route と rms_norm_gated は無効、n-gram は
backend=pread / threads=12 / 4bit、HTTP を挟まない、temp=0、1 回 64 トークン)。
測定は `tools/bench_shipping.py`。

**この数字に投機は一切入っていない。**Flash-Next (qwen4_exp) は SpecEngine の
契約を満たさず `FallbackRunner` に落ちる。README の「自己投機 1.5-2.2 倍」は
Qwen3.8-27B レーンの数字で、この値には掛けられない。公開時に混ぜないこと。

見込みの 30.6ms には届かなかった。32.67 から pread の -2.1ms を引く計算が
二重取りだった。

#### pread の -2.1ms は出荷経路には乗らない

同じプロセス内で交互に振って測ると、測り方で結果が割れる。

| 測り方 | pread | mmap | 差 |
|---|---|---|---|
| 同期ループ (model() を直接呼び毎トークン argmax)、融合あり | 32.93 | 34.54 | -1.61 |
| 同期ループ、融合なし | 49.03 | 50.64 | -1.61 |
| build_runner 経路 (stream_generate) | 32.51 | 32.48 | +0.03 |

-2.1ms を出したのは 1 段目と同じ形の測定 (`tools/ngram_backend_ab.py`) で、
そこでは今も再現する。出荷経路では消える。stream_generate は次のトークンを
非同期で進めるので、n-gram の行読み出し (ホスト側の pread とフォールト) が
GPU の計算に重なって隠れる。同期ループは毎トークン argmax でホストへ降りる
ため重ならず、読み出しがそのまま表に出る。

pread は既定のまま残す。中央値では差が出ないが、上振れの尾は mmap の方が
長い (mmap の IQR 32.25-33.38 に対し pread は 32.43-32.53)。

#### 測り方の落とし穴: 同じプロンプトを測り直さない

greedy なら同じトークン列が出るので、同じプロンプトを繰り返すと毎回同じ
n-gram 行 (64 トークンで 1024 行 = 100KB) しか触らず、2 回目以降はページ
キャッシュから返る。32GB の表から毎トークン 16 行をランダムに引く部品を、
温めた状態で測ることになる。`tools/bench_shipping.py` は 1 計測につき未使用の
プロンプトを 1 本使う。実測では温冷の差は 32.18 対 32.34 で 0.16ms しかなく、
今回の結論は変わらなかった。

### v-96 は 128GB 機では採らない

3.08ms を KLD +0.02524 で買う取引で、**122 ms/KLD**。今日採用した取引は
gdn 8->6 が 2286、head が 1073、attn が 500、gdn 8->4 が 434。却下した
hc 8->4 が 67。122 は却下側。

尺度として、**n-gram 表を丸ごと捨てた場合の KLD が 0.04795**。v-96 の
0.02902 は、51.2B の表を全部捨てるのの半分以上を 3ms のために払っている。

札別でも全面的に悪化し、attn +0.035 / reason +0.034 / experts +0.031 と
判断・推論側が最も落ちる。用途で救われる配分ではない。

96GB Mac 向けの選択肢としては有効で、これで品質の数字が付いた。

### 判定の順序について

**交換レートの基準は数字が出る前に宣言してから測った。**後から基準を決めると
都合よく読めてしまう。v-96 は事前基準で却下圏に落ちた。

### 外部実装から持ち込んだ仮説の扱い

`ddalcu/mlx-serve` (MIT、Zig) を読んで 3 つ持ち込み、**1 勝 2 敗**。

| 仮説 | 結果 |
|---|---|
| n-gram の mmap フォールト直列化 -> 並列 pread | **当たり** (-2.1ms、ビット単位一致 240/240) |
| stock gather_qmm が理想の 3 倍動かす | 外れ。3.33x は top_k=1 のときだけで、バイト比ではなく固定費。実際の top_k=10 では 1.44x、上限 1.5ms |
| lm_head は巨大 N のタイル問題 | 外れ。N を分割しても速くならない |

外した 2 つは、**こちらの実測に基づく解釈を、あちらの環境の数字で上書きして
しまった**もの。lm_head については「差の 3ms は他の部品と帯域を奪い合うぶん」と
最初に正しく書いていたのに、後から読んだコメントで置き換えた。

外部実装は仮説の供給源として有益 (pread がそれ)。ただし**こちらの実測を
上書きさせない**こと。持ち込むときは「未検証の仮説」として渡し、着手順を
変えろとまでは書かない。
