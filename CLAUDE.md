...
- 強冷却は常時運用せず、事前検査と通常の同一process A/Bを通った候補の最終比較に使う。
  ユーザーが用意できるのは1日最大2回、各数時間。使うときは開始前に、対象モデル・固定する条件・
  所要時間をチャットで明示する。原則1枠はFlash-Nextの研究目標候補、もう1枠は別モデルで
  その日に生き残った候補へ充て、弱い候補の探索では消費しない (ユーザー 2026-09-04 18時台)。
- ハードウェア分岐は M1〜M5 の既知世代だけを列挙して将来世代を無効にしない。M6 は M5 の
  後継として、まず M5/NAX 向けの保守設定を継承する。カーネル固有の threadgroup 数・明示
  threadgroup memory・形状条件を実行前に検査し、コンパイル不能または資源条件外なら素へ戻す。
  M6 実機の速度表が得られたら閾値だけを上書きし、安全条件と fallback は共通のままにする。
...
- knob の既定値は 3 種類を区別すること。同じ「既定 off」でも中身が違う。
  ...
  **I/O を含む経路の A/B は、同じ機体で続けて測るとページキャッシュを共有する** (プロセスを分けても同じ)。
  ...
  `MLXTURBO_QSA_BLOCKS64` (既定 on、`=0` で off) はK2bだけをKV長16,000〜18,000で64 blocksにする。17kでround -1.0% / ms-token -4.8%。4kは+6.5%のため、50kは品質低下のため従来表へ戻す。
  ...
  実測した構造（40層、hidden 2,048、256 experts、top-8、full-attention間隔4、head dim 256、vocab 248,320）の
  Qwen3.6-35B-A3B (`qwen3_5_moe`) + MTP の未指定時だけ、入力3,830 token以上は
  `n_draft/max_draft/lookup_len=3/3/0`、未満は従来の3/8/16。temp=0.7の4k/17k/50kは
