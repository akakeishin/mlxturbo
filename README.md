...
- **テキスト専用。画像・音声・動画は扱えない。** 元の checkpoint は VLM だが、この変換は vision タワーを含んでいない（変換後の重みに vision 系テンソルは 0 個）。`image_url` / `image` / `input_audio` などの非テキストブロックを含むリクエストは 400 で断る。対応の見通しは [`docs/BACKLOG.md`](docs/BACKLOG.md) の §1 を参照
- **1 プロセス 1 モデル。** モデルは起動時に 1 回だけロードして常駐する設計で、複数モデルの切り替えは別プロセスを立てる必要がある
- **既定ではリクエストを直列処理する。** まとめるフラグは対象ごとに 2 つあり、どちらも既定は 1（直列）。`--max-batch N` は継続バッチング（`FallbackRunner` に載るリクエストが対象）、`--max-batch-spec N` はバッチ x 投機（Flash-Next + MTP のまま同時要求をまとめる）。後者は「サンプリング設定が同じ」「プロンプト長 + max_tokens が 2048 に収まる」「長さの比が 1.5 倍以内」の要求だけが対象で、外れたものは直列のまま。待ち行列上限 `--max-queue` を超えると 503
- **spec / flash_spec 経路に非恒等値のサンプリングパラメータが来ると、そのリクエストだけ非投機に降格する。** 投機のブロック検証が厳密な分布一致を前提にしているため。降格したことはレスポンスの `downgrade_reason` とサーバーログに出る
- **logprobs は非投機経路のみ。** 投機経路で要求されると上と同じく降格して返す。ストリーミングとの併用は 400
