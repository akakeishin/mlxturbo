...
[[kernel]] void custom_kernel_base_m12_bf16(
  ...
  uint3 threadgroup_position_in_grid [[threadgroup_position_in_grid]]) {
    ...
    for (int group = (int)sg; group < groups; group += SPLITS) {
        ...
        for (int kt = 0; kt < QGROUP / 8; ++kt) {
            ...
            // representable as half for the normed activation range, while
            // half preserves three more dequantized-weight mantissa bits.
            a1.thread_elements()[0] = frag_row < 4
                ? half(x[(size_t)(8 + frag_row) * K + k_base + frag_col])
                : half(0);
            a1.thread_elements()[1] = frag_row < 4
...
template [[host_name("custom_kernel_base_m12_bf16_bfloat16_t_5120_17408")]] [[kernel]] decltype(custom_kernel_base_m12_bf16<bfloat16_t, 5120, 17408>) custom_kernel_base_m12_bf16<bfloat16_t, 5120, 17408>;
